# Apex Insulation Pros — Website

Official marketing site for **Apex Insulation Pros**: spray foam, blown-in, radiant barrier, insulation removal, and attic or wall work for **Florida homes and buildings**. We are based in **St Augustine** on the First Coast and serve **Jacksonville & Northeast Florida**, **Orlando & Central Florida**, and **Tampa & Southwest Florida**, plus surrounding markets. This repository is the source for the live site and its ongoing updates.

---

## Why this site feels faster than typical contractor websites

Many contractor sites run on heavy WordPress themes, drag-and-drop builders, or stacks of plugins. That often means large JavaScript bundles, slow first loads on phones, and fragile performance when something updates.

**This site is different by design.**

| Typical contractor site | This site |
|-------------------------|-----------|
...
JavaScript is still available where it truly helps (for example the on-demand map in the service area section); it is not loaded “because the platform always loads it.”
...
```bash
npm install
cp .env.example .env    # set R2 / SITE_URL / optional Google Maps keys as needed
...
├── components/          ← Header, Hero, services, reviews, contact, map, footer, etc.
├── pages/
│   ├── index.astro
│   ├── contact.astro
│   ├── service-areas.astro
...
The main **contact** form in `ContactSection.astro` uses Netlify-style attributes (`data-netlify="true"`). For **Cloudflare Pages**, wire submissions through [Pages Functions](https://developers.cloudflare.com/pages/functions/) or a provider such as Formspree, Web3Forms, or EmailJS—adjust the `action` / handler as needed.

The **homepage hero** estimate block lives in `HeroEstimateForm.astro`; align its submission path with the same approach you use for the contact form.
...
- **Service areas & map** — `SITE.serviceAreas`, `SITE.map`, and `MapSection.astro`.  
...
*Stack: Astro, static output, Cloudflare Pages–friendly. Built for speed, clarity, and long-term maintainability — for **Apex Insulation Pros** and Florida customers from the First Coast to Tampa.*
