# Apex Insulation Pros — marketing site

Astro site for **Apex Insulation Pros** (St Augustine, FL). Business copy, services, colors, and media URLs live in `src/config/site.ts`. Set `SITE_URL` in `.env` for canonical URLs and the sitemap.

Starter template: data-driven Astro + Netlify-style forms; deploy as you prefer (e.g. Cloudflare Pages).
